...
describe('solutionSlice', () => {
  describe('reducers', () => {
    const initialState = { solutions: [], selectedSolutions: [] };
    const payloadValue = {
      id: 1,
      evaluation: 100,
      status: 1,
    ...
    it('sets the solutionRead payload', () => {
      ...
      expect(state.solutions).toEqual([payloadValue]);
    });
    ...
    describe('deleteSolution', () => {
      ...
      describe('should handle reducer without error', () => {
        ...
        it('solutionReducer', async () => {
          ...
          /* THEN */
          const state = store.getState().problem.solutionReducer;
          expect(state).toEqual({ solutions: [], selectedSolutions: [] });
        });
      });
    });
    describe('readSolutionOfOthers', () => {
      describe('should handle response validity', () => {
        const selectedUsersList = [
          { user_id: 1, username: 'hi' },
          { user_id: 2, username: 'hello' },
          { user_id: 3, username: 'bye' },
        ];
        const problemId = 1;
        let store;
        beforeEach(async () => {
          /* GIVEN */
          request.get = jest.fn();
          request.get.mockReturnValue({ data: ['hi'] });
          request.post = jest.fn();
          request.put = jest.fn();
          request.delete = jest.fn();
          store = configureStore().store;
        ...
        necessaryKeysInResponse.forEach((key) => {
          it(`key "${key}" does not exist.`, async () => {
            ...
            await store.dispatch(
              readSolutionOfOthers(selectedUsersList, problemId)
            );

            /* THEN */
            expect(request.get).toHaveBeenCalledTimes(3);
            expect(request.get).toHaveBeenCalledWith(
              `problem/${problemId}/solution/${1}`
            );
          });
        });
      });
